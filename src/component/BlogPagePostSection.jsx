  ...
  const fetchBlogs = async (pageNumber = 1) => {
    try {
      ...
      const res = await axios.get(
        `https://api.plannedrations.com/api/web/get-blogs?page=${pageNumber}`,
      );
          ...
          <div className="grid ssm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 ssm:gap-5 ml:gap-8">
            {filteredPosts.map((post) => (
              <Link
                to={`/blog/${post.id}`}
                key={post.id}
                className="bg-transparent overflow-hidden cursor-pointer flex flex-col transition-all duration-300 hover:scale-105 active:scale-95"
              >
                <img
                ...
                <div className="flex flex-col flex-grow py-3">
                  ...
                  <div className="mt-auto border-t border-brand-planoff pt-1 ">
                    <div
                      className="w-full flex justify-between items-center text-sm font-bold text-brand-offpurple"
                    >
                      <div>Read Blog</div>
